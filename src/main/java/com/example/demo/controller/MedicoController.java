...
public class MedicoController {
	...
	@Operation(summary = "Atualizar médico", description = "Atualizar médico no sistema")
	@PatchMapping("/{id}")
	public ResponseEntity<ApiResponse<MedicoDTO>> atualizarMedico(@PathVariable Long id,
			@RequestBody MedicoDTO medicoDTO) {
		try {

			MedicoDTO medicoParaSalvar = medicoService.buscarPorId(id)
					.orElseThrow(() -> new IllegalArgumentException("Médico não encontrado!"));

			medicoParaSalvar.setCrm(medicoDTO.getCrm());
			medicoParaSalvar.setEspecialidade(medicoDTO.getEspecialidade());
			medicoParaSalvar.setNome(medicoDTO.getNome());

			medicoService.atualizar(medicoParaSalvar, id);
			
			ApiResponse<MedicoDTO> response = new ApiResponse<>(medicoParaSalvar);
